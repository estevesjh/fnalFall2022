""" Setup files and bins

- Set the lambda/redshift bins
- Set the treecorr param file
- Set the output file systems
"""
import os
import numpy as np

# setup bins
lbd_bins = np.array([5, 20, 30, 45, 60, 1000])
z_bins = np.array([0.2, 0.35, 0.5, 0.65])

# setup treecorr
config ={
         'dec_col': 'dec',
         'dec_units': 'degrees',
         'ra_col': 'ra',
         'ra_units': 'degrees',
         'sep_units': 'arcmin',
         'max_sep': 100,
         'min_sep': 1,
         'nbins': 20,
         'verbose': 0
        }

# 3d metric
config3 = {
        'nbins': 31,
         'max_sep': 200,
         'min_sep': 4,
         'min_rpar': -200,
         'max_rpar': 200,
         'verbose': 0
        }

# setup output file system
class SetupFiles(object):
    def __init__(self, tag, Npatches, root='./'):
        self.root = root
        self.Npatches = Npatches
        self.root_out = self.root+'nbody_output/'
        self.root_dir = self.root_out+'kmeans%i/'%(self.Npatches)
        self.root_tmp = self.root_out+'tmp/'
        
        # initialize variables
        self.setup_files(tag)
        
        # make dirs
        self.make_roots()
        
    def setup_files(self, tag):
        self.label = tag
        self.outfile = self.root_out+self.label+'.npz'
        self.tmp_fname_base = self.root_out+'xi_'+self.label+'_k%i'
        self.fname_kmeans_centers= self.root_tmp+self.label+'_centers.npy'
        
        # create root directories
        self.make_roots()
        
    def get_outfile(self, lbd_bin, z_bin):
        fname = '%s_l%i-z%i.npz'%(self.label, lbd_bin, z_bin)
        return self.root_dir+fname
    
    def write_tmp_files(self, clist, rlist, tag, n_patches):
        files1, files2 = [], []
        for k in range(n_patches):
            f1 = self.root_tmp+'data_%s_k%i.fits'%(tag, k)
            f2 = self.root_tmp+'rnd_%s_k%i.fits'%(tag, k)
            clist[k].write(f1, overwrite=True)
            rlist[k].write(f2, overwrite=True)
            files1.append(f1)
            files2.append(f2)
        return files1, files2

    def make_roots(self):
        self.make_dir(self.root)
        self.make_dir(self.root_out)
        self.make_dir(self.root_dir)
        self.make_dir(self.root_tmp)
        
    def make_dir(self, dirname):
        if not os.path.isdir(dirname):
            os.mkdir(dirname)