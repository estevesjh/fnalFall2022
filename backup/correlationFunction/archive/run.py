...
fname_base = './data/xi_Nside{}_jk_patches_{:05d}.npy'
...
jk =  JackKnifer(Nside, hpxmap, frac_thr= 0.7)
...
# export PYTHONPATH="${PYTHONPATH}:/global/homes/j/jesteves/.local/lib/python2.7"

# alias lsst='source /software/lsstsw/stack/loadLSST.bash && setup lsst_distrib && export PYTHONPATH="${PYTHONPATH}:/usr/lib64/python3.6/site-packages:$HOME/repos/AuxTel_imaging_survey" && setup -k -r $HOME/repos/obs_lsst && setup -k -r $HOME/repos/pipe_tasks && setup -j rapid_analysis -r $HOME/repos/rapid_analysis && setup -j atmospec -r $HOME/repos/atmospec && setup -j drp_pipe -r $HOME/repos/drp_pipe'